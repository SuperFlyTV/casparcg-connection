...
describe('Test the CGCLEAR command', () => {
	...
	test('Check CGUPDATE v218', async () => {
		version('218')
		let reqPromise = conn.cgUpdate({ channel: 1, layer: 1,
		  flashLayer: 1, data: 'wibble' })
	...
	test('Check CGUPDATE v207', async () => {
		version('207')
		let reqPromise = conn.cgUpdate({ channel: 1, layer: 1,
		  flashLayer: 1, data: 'wibble' })
		await expect(reqPromise).resolves.toMatchObject({
	...
	test('Check CGUPDATE v220', async () => {
		version('220')
		let reqPromise = conn.cgUpdate({ channel: 1, layer: 1,
		  flashLayer: 1, data: 'wibble' })
		await expect(reqPromise).resolves.toMatchObject({
