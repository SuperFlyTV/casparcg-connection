...
export class CasparCGSocket extends EventEmitter implements ICasparCGSocket {
	...
	private _commandTimeout = 5000 // @todo make connectionOption!
	...
	public executeCommand(command: IAMCPCommand): IAMCPCommand {
		...
		if (this.queueMode === QueueMode.SALVO)
			commandString =
				`REQ ${command.token} ` +
				(command.constructor as any)['commandString'] +
		...
		if (this._commandTimeoutTimer) global.clearTimeout(this._commandTimeoutTimer)
	...
	private _onConnected() {
		...
		if (this._client) {
			_(this._client)
				.splitBy(/(?=\r\n)/)
				.errors((error: Error) => this._onError(error))
				.each((i: unknown) => this._parseResponseGroups(i as string))
			this.connected = true
		}
	}
	...
	private _parseResponseGroups(i: string): void {
		...
		if (AMCPUtil.CasparCGSocketResponse.evaluateStatusCode(i) === 200) {
			...
			AMCPUtil.CasparCGSocketResponse.evaluateStatusCode(i) === 201 ||
			AMCPUtil.CasparCGSocketResponse.evaluateStatusCode(i) === 400 ||
			AMCPUtil.CasparCGSocketResponse.evaluateStatusCode(i) === 101
		) {
			this._parsedResponse = new AMCPUtil.CasparCGSocketResponse(i)
			this._commandTimeoutTimer = global.setTimeout(() => this._onTimeout(), this._commandTimeout)
			return
		} else if (
			(this._parsedResponse && this._parsedResponse.statusCode === 201) ||
			(this._parsedResponse && this._parsedResponse.statusCode === 400) ||
			(this._parsedResponse && this._parsedResponse.statusCode === 101)
		) {
			this._parsedResponse.items.push(i)
			this.emit(CasparCGSocketResponseEvent.RESPONSE, new CasparCGSocketResponseEvent(this._parsedResponse))
			this._parsedResponse = undefined
			...
			const parsedResponse: AMCPUtil.CasparCGSocketResponse = new AMCPUtil.CasparCGSocketResponse(i)
